...
def find_nth_byte_of_modified_CT(
    byte_index_to_replace: int, attack_ct: bytes, wrong_i: int
) -> int:
    """
    ...
    for i in range(0, 256):
        attack_ct = replace_byte(
            attack_ct, byte_index_to_replace, i
        )  # replace the n_th byte
        ...
        if response.text not in response_messages:
            if byte_index_to_replace == 0:
                print(
                    "FUNC_find nth byte: we are at byte index to replace 0 for this block"
                )
                return i
            ...
            response2 = make_request(
                f"{BASE_URL}/quote/", {"authtoken": attack_ct.hex()}
            )
    ...
    raise PaddingOracleException(
        f"No valid padding found for byte index {byte_index_to_replace}"
    )


def decrypt_one_block(
    n: int,
    ...
    for i in range(1, 17):
        ...
        for j in range(1, i):
            ...
            c_j_x = ct_x[
                idx_j
            ]  # extract the found ct byte that results in valid padding at byte+16
        ...
        decrypted_plaintext = replace_byte(
            decrypted_plaintext, idx + 16, p_62
        )  # bookkeeping
        ...
        print(
            f"FUNC_decrypt_one_block: y_62_actual = {y_62_actual} = {p_62} xor {original_token[idx]}"
        )
...
def padding_oracle_attack(original_token, no_of_blocks):
    ...
    for i in range(0, no_of_blocks):
        decrypted_plaintext, ct_x, key_stream = decrypt_one_block(
            no_of_blocks - i - 1,
            decrypted_plaintext,
            ct_x,
            original_token[0 : (no_of_blocks - i) * blocklength],
            no_of_blocks,
            key_stream,
        )
...
def forge_cbc_token(plaintext):
    ...
    print(
        f"MAIN: forged_ct_length = {forged_ct_length}"
    )  # when this is 64 CT = IV || CT_0 || CT_1|| CT_2(random) ie 4 blocks
    ...
    for i in range(n - 1, 0, -1):  # i = N-1 = 2 when N = 3
        ...
        decrypted_plaintext, ct_x, key_stream = decrypt_one_block(
            ...
            decrypted_plaintext,
            ct_x,
            bytes(forged_token[0 : blocklength * (i + 1 + 1)]),
            0,
            key_stream,
        )
        d_PO_c_2 = key_stream
    ...
    decrypted_plaintext, ct_x, key_stream = decrypt_one_block(
        ...
        decrypted_plaintext,
        ct_x,
        bytes(forged_token[0 : blocklength * 2]),
        0,
        key_stream,
    )
    d_PO_c_0 = key_stream
...
def main():
    ...
    print(
        f"ATTACK: auth_token[hex]: {auth_token}, len:{len(auth_token)}, blocks:{len(auth_token) / 32}\n"
    )
    ...
    # ======================================
    # Step 2: launch the CBC-padding-oracle-decryption attack to get the secret comment out/ uncomment to disable or run
    decrypted_plaintext, ct_x = padding_oracle_attack(original_token, no_of_blocks)

    # ======================================================

    # Step 3: copy paste the secret you got from Step 2(hard coded here) #to do (if time): regex it out of the padding_oracle_attack
    # and launch the decryption oracle attack to forge a tokem
    ...
    response_final = make_request(
        f"{BASE_URL}/quote", {"authtoken": bytes(forged_token).hex()}
    )
